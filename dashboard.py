...
def abrir_dashboard(usuario, rol, login_root=None):
    # Ocultar la ventana de login si existe
    if login_root:
        login_root.withdraw()

    root = tk.Toplevel()
    root.title("Sistema Médico — Dashboard Completo")
    ...
    def _tarjeta_medico(parent, m, row, col, refresh_fn):
        ...
        tk.Button(inner, text="🗑 Eliminar", command=eliminar, bg="#fff0f0", fg=COLOR_DANGER, font=("Segoe UI", 8, "bold"), relief="flat", padx=8, pady=3, cursor="hand2").pack(anchor="e", pady=(10, 0))

    # Tarjeta Enfermedad (Con soporte para síntomas)
    def _tarjeta_enfermedad(parent, e, row, col, refresh_fn):
        ...
        try:
            ...
            if sintomas_db:
                lista_nombres = [s["nombre"] for s in sintomas_db]
                texto_sintomas = "Síntomas: " + ", ".join(lista_nombres)
            else:
                texto_sintomas = "Sin síntomas registrados"
        except Exception:
            texto_sintomas = "Error al cargar síntomas"

        tk.Label(inner, text=texto_sintomas, font=("Segoe UI", 8, "italic"), bg=COLOR_WHITE, fg=COLOR_ACCENT, wraplength=190, justify="left").pack(anchor="w", pady=(5,0))
    ...
    # Tarjeta Registro Médico (Para Diagnóstico e Historial)
    ...
    def modal_agregar_diagnostico(refresh_fn):
        ...
        def guardar():
            ...
            try:
                ...
                db.agregar_diagnostico(paciente, enf, prob_float)
                db.cargar_todo()
                modal.destroy()
                refresh_fn()
            except ValueError:
    ...
    def _crear_vista_base(titulo, btn_texto, btn_cmd, datos, fn_tarjeta, refresh_fn):
        ...
        if btn_texto:
            tk.Button(barra, text=f"＋ {btn_texto}", command=lambda: btn_cmd(refresh_fn), bg=COLOR_ACCENT, fg=COLOR_WHITE, font=FONT_BTN, relief="flat", padx=14, pady=6, cursor="hand2").pack(side="right")

        grid = tk.Frame(scroll_frame, bg=COLOR_BG)
        grid.pack(fill="both", padx=24, pady=4)

        if not datos:
            _tarjeta_vacia(grid, f"No hay registros aquí", "Usa el botón superior para empezar" if btn_texto else "")
        else:
    ...
    def vista_inicio():
        ...
        tk.Label(scroll_frame, text=f"Bienvenido, {usuario} 👋", font=("Segoe UI", 20, "bold"), bg=COLOR_BG, fg=COLOR_TEXT).pack(anchor="w", padx=30, pady=(30, 4))
    ...
    def _nav_btn(texto, comando):
        btn = tk.Button(sidebar, text=texto, bg=COLOR_SIDEBAR, fg="#c2ccd8", bd=0, font=("Segoe UI", 10), activebackground="#243447", activeforeground=COLOR_WHITE, anchor="w", padx=18, pady=9, relief="flat", cursor="hand2")
        def click(b=btn, cmd=comando):
            if current_btn[0] and current_btn[0] != b: current_btn[0].config(bg=COLOR_SIDEBAR, fg="#c2ccd8")
    ...
    if rol == "medico":
        ...
        _nav_btn("🧠 Diagnósticos", vista_diagnostico)
    ...
    tk.Button(sidebar, text="⬅ Cerrar sesión", command=cerrar_sesion, bg="#c0392b", fg=COLOR_WHITE, font=("Segoe UI", 10, "bold"), relief="flat", pady=10, cursor="hand2").pack(fill="x", side="bottom")

    vista_inicio()
    root.mainloop()

# Bloque para pruebas directas si lo ejecutas sin login.py
